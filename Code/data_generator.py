...
class DataGenerator_Base(Sequence):
            ...
            if self.chunk_level_datapoints:
                ...
                for i in range(nr_post_groups):
                    ...
                    self.indexes_per_user[u].append(range(i * self.posts_per_group + self.post_offset,
                                                          min((i + 1) * self.posts_per_group + self.post_offset,
                                                              len(user_posts))))
...
class DataGenerator_BERT(Sequence):
    'Generates data for Keras'
    ...
    def __encode_text__(self, tokens, raw_text):
        ...
        return (encoded_token_ids, encoded_token_attnmasks, encoded_emotions, encoded_pronouns, encoded_stopwords, encoded_liwc,
                )
    ...
    def __data_generation_hierarchical__(self, users, post_indexes):
        ...
        for subject in users:
            ...
            for i, words in enumerate(all_words):
                ...
                for p, posting in enumerate(words):
                    encoded_token_ids, encoded_token_attnmasks, encoded_emotions, encoded_pronouns, encoded_stopwords, encoded_liwc, \
                        = self.__encode_text__(words[p], raw_text[p])
                    if 'liwc' in self.data[subject] and not self.compute_liwc:
                        liwc = liwc_scores[i][p]
                    else:
                    ...
                    try:
                        ...
                        subject_id = subject
                    tokens_ids_data.append(encoded_token_ids)
                    tokens_attnmasks_data.append(encoded_token_attnmasks)

                    categ_data.append(encoded_emotions + [encoded_pronouns] + liwc)
                    sparse_data.append(encoded_stopwords)

                # For each range
                user_token_ids.append(tokens_ids_data)
                user_token_attnmasks.append(tokens_attnmasks_data)
        ...
        user_token_ids = pad_sequences(user_token_ids,
                                             maxlen=self.posts_per_group,
                                             value=self.pad_value)
        user_token_attnmasks = pad_sequences(user_token_attnmasks,
                                             maxlen=self.posts_per_group,
                                             value=self.pad_value)

        user_token_ids = np.rollaxis(np.dstack(user_token_ids), -1)
        user_token_attnmasks = np.rollaxis(np.dstack(user_token_attnmasks), -1)
...
class DataGenerator_BERT_TEST(Sequence):
    'Generates data for Keras'
    ...
    def __data_generation_hierarchical__(self, users, post_indexes):
        ...
        for subject in users:
            ...
            encodings = self.tokenizer.batch_encode_plus(
                ...
                max_length=hyperparams['maxlen'],
                padding='max_length',
                truncation=True,
                return_attention_mask=True,
                return_tensors='tf',
            )
            # Extract token IDs and attention masks from the encodings
            encoded_token_ids = encodings['input_ids']
            encoded_token_attnmasks = encodings['attention_mask']

            # Pass the entire batch through the model to get embeddings
            embedded_texts = self.model(encoded_token_ids, attention_mask=encoded_token_attnmasks,
                                        output_hidden_states=True, return_dict=True)['hidden_states'][-4:]
            # Sum the embeddings across layers
            embedded_texts = tf.add_n([layer for layer in embedded_texts])

            # Reshape and pad the data
            num_posts = len(all_posts)
            post_length = embedded_texts.shape[1]
            num_features = embedded_texts.shape[2]
            embedded_texts = tf.reshape(embedded_texts, (num_posts, post_length, num_features))
            print(f"embedded_texts.shape: {embedded_texts.shape}")
            ...
            for i, words in enumerate(all_words):
                ...
                for p, posting in enumerate(words):
                    ...
                    # embedded_post = self.model(encoded_token_ids, attention_mask=encoded_token_attnmasks,
                    #                                         output_hidden_states=True, return_dict=True)[
                    #                                                                'hidden_states'][-4:]
                    # print(encoded_token_ids.shape)
                    # print(tf.add_n([layer for layer in embedded_post]).shape) #(8,256,768)
                    # embedded_post = tf.add_n([layer for layer in embedded_post], axis=0)
                    #
                    # embedded_texts.append(embedded_post)
                    # tokens_ids_data.append(encoded_token_ids)
                    # tokens_attnmasks_data.append(encoded_token_attnmasks)

                    #print(f"per post? {len(tokens_ids_data)}")
                ...
                user_token_ids.append(tokens_ids_data)
                user_token_attnmasks.append(tokens_attnmasks_data)
                #user_embedded_texts.append(embedded_texts)

                #print(f"per subject? {len(tokens_ids_data)}")
        ...
        user_embedded_texts = pad_sequences(user_embedded_texts,
                                             maxlen=self.posts_per_group,
                                             value=self.pad_value)

        user_embedded_texts = np.rollaxis(np.dstack(user_embedded_texts), -1)
        user_token_ids = np.rollaxis(np.dstack(user_token_ids), -1)
        user_token_attnmasks = np.rollaxis(np.dstack(user_token_attnmasks), -1)
        ...
        labels = np.array(labels, dtype=np.float32)
        print(user_token_ids.shape)
        print(user_embedded_texts.shape)
